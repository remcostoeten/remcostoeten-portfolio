...
const IconGrid = () => {
    ...
    return (
        <div className="grid w-full mx-auto animate-fadeIn">
            ...
            <div className="grid md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-4 pl-4 pr-0 gap-x-0 py-10 gap-y-4">
                {Icons.map((item, index) => (
                    <Link href={item.url} className="flex relative group mx-auto  bg-white border-2 border-primary rounded-md p-8">
                        <div className="flex min-h-[110px] max-h-[110px] w-[110px] items-center justify-center h-full display" ref={iconRefs.current[index]} key={index}>
                            {item.icon}
                        </div>
                    </Link>
                ))}
