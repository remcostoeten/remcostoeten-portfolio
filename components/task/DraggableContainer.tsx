...
export default function DraggableContainer({ tasks, updateTask, removeTask }: DraggableContainerProps) {
    ...
    const updateExistingTask = async (title: string, description: string, category: string, taskId: string) => {
    ...
    return (
        <>
            {removeConmfirm && (
                <Alert className="confirm-task" variant="outlined" severity="warning">
                    Are you sure you want to delete this task?
                    <button className="btn btn--primary" onClick={() => editedTask && confirmDelete(editedTask.id)}>
                        Duh!
                    </button>
                ...
                {lanes.map((lane) => (
                    <div key={lane.id} className="tasks__lane">
                        <div className="tasks__header">
                            <div className="title">
                                {lane.title} <span>({tasks.filter((task) => task.status === lane.id).length})</span>
                            </div>
                            {/* <span className='add'>
								<AddBoxRoundedIcon
                        ...
                        <TaskModal
                            ...
                            onSubmit={(title, description, category, taskId) => {
                                if (taskId) {
                                    updateExistingTask(title, description, category, taskId);
                                } else {
                                                ...
                                                {(provided) => (
                                                    <div className="task" ref={provided.innerRef} {...provided.draggableProps} {...provided.dragHandleProps}>
                                                        <div className="tasks__bottom">
                                                            <div className="left">{task.category}</div>
                                                            <div className="right">
                                                                <DeleteForeverIcon className="remove" onClick={() => confirmDelete(task.id)} />
                                                                <Edit className="edit" onClick={() => openEditModal(task)} />
                                                            </div>
                                                        </div>
