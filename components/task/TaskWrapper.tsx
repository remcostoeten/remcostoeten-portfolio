...
export default function TaskWrapper() {
    ...
    const handleSubmit = async (title: string, description: string, category: string, taskId?: string) => {
        if (taskId) {
                        ...
                        <main>
                            <div className="todo__header flex flex-col mb-2">
                                <h2 className="text-xl font-semibold">
                                    Welcome back, <span className="text-blue-500">{auth.currentUser?.displayName} 👋</span>
                                </h2>
                                <div className="todo__intro">
                                    <div className="text">
                                        {tasks.length === 0 && (
                                            <>
                                                <p>You have no tasks left. Time to relax. 🥳 Or get busy and create some new ones!</p>
                                            </>
                                        )}
                                        {tasks.length === 1 && (
                                            <>
                                                <p>You've got {tasks.length} task left. Good luck nailing it! 🤑</p>
                                            </>
                                        )}
                                        {tasks.length >= 2 && tasks.length <= 4 && (
                                            <>
                                                <p>
                                                    ...
                                                    {tasks.length === 1 ? '' : 's'} left. Better start working then! 😳
                                                </p>
                                            </>
                                        )}
                                        {tasks.length > 4 && <p>You've got {tasks.length} task(s) left. 😵‍💫 But no pressure, I wont judge you slacking. 🫣</p>}
                                    </div>
                                </div>
                                <div className="toggle-view flex space-x-4">
                                    <div className="flex">
                                        <button className="text-sm board-view text-black bg-blue-500 font-semibold py-2 px-4 rounded-md" onClick={toggleView}>
                                            Board view
                                        </button>
                                        <button className="text-sm list-view bg-blue-500 text-black font-semibold py-2 px-4 rounded-md" onClick={toggleView}>
                                            List view
                                        </button>
                                    </div>
                                    <div className="mb-2 mr-2 rounded bg-violet-400 text-white font-semibold py-1 text-sm px-4 cursor-pointer mb-0" onClick={() => setIsModalOpen(true)}>
                                        {' '}
                                        Add task
                                    </div>
                                </div>
                            ...
                            <div className={`view-container ${view}-view`}>
                                ...
                                {view === 'board' && (
                                    <div className="tasks">
                                        <DraggableContainer tasks={tasks} updateTask={updateTask} removeTask={removeTask} />
                                    </div>
                                )}
                                {/* List View */}
                                {view === 'list' && (
